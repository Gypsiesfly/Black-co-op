...
export function Navbar({ activePath }: NavbarProps) {
  ...
  // Show a loading state that matches the navbar's initial state
  if (!mounted) {
    return (
      <div className="bg-black w-full h-16 fixed top-0 left-0 z-50">
        {/* Empty div with same dimensions as the navbar */}
      </div>
    );
  }
  ...
  return (
    <div className="font-passion-one">
      <div className="fixed top-0 left-0 w-full z-[1000]">
        ...
        {/* Main Navigation */}
        <nav className={`bg-black py-3 px-4 md:px-8 flex items-center justify-between ${mobileMenuOpen ? 'h-screen' : 'h-16'}`}>
          ...
          {/* Mobile Menu */}
          <div
            className={`fixed top-0 right-0 h-full w-full sm:w-80 bg-black transform transition-transform duration-300 ease-in-out z-[1000] ${
